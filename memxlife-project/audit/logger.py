...
class AuditLogger:
    ...
    def generate_report(
        ...
        # Physical consistency validation section
        if consistency:
            lines.extend(["## 6. Physical Consistency Validation", ""])
            is_ok = consistency.get("is_consistent", True)
            lines.append(
                f"**Status**: {'✅ All checks passed' if is_ok else '❌ Violations detected'}"
            )
            lines.append("")
            ...
            cross_checks = consistency.get("cross_checks", [])
            if cross_checks:
                lines.append("### Cross-Checks")
                for cc in cross_checks:
                    check_name = cc.get("check", "unknown")
                    passed = cc.get("passed", "N/A")
        ...
        # Environment fingerprint section
        if fingerprint:
            lines.extend(["## 7. Environment Fingerprint Analysis", ""])
            tampered = fingerprint.get("tampering_detected", False)
            lines.append(
                f"**Trust Level**: `{fingerprint.get('trust_level', 'unknown')}`"
            )
            lines.append(
                f"**Tampering Detected**: {'⚠️ YES' if tampered else '✅ No'}"
            )
            lines.append("")
            ...
            lines.append(
                f"**Recommendation**: {fingerprint.get('recommendation', 'N/A')}"
            )
            lines.append("")
    ...
    def save_report(
        ...
        report = self.generate_report(results, environment, consistency, fingerprint)
