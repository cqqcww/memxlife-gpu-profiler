...
curl --connect-timeout "${COURSE_CURL_CONNECT_TIMEOUT}" --max-time "${COURSE_CURL_MAX_TIME}" -sS -X POST "http://${SERVER}:8080/submit3" \
  -H "Content-Type: application/json" \
  -d "{\"id\":\"${USER_ID}\",\"gpu\":1}" | tee "${TMP_JSON}"
echo

python3 - "${TMP_JSON}" "${REPO_ROOT}/output_id3.txt" <<'PY'
import json
import re
import sys
