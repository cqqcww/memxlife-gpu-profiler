...
def heuristic_candidates() -> list[CandidateConfig]:
    return [
        ...
        CandidateConfig(
            ...
            accumulation_order="mainfirst",
            allow_tf32=False,
            cache_mode="hybrid_weff_threshold2_btcache",
            variant_name="aten_addmm_inplace_btcontig_mainfirst_hybridweff_threshold2_btcache",
            notes="Delay W_eff materialization until the second same-weight varying-X event, but cache a contiguous B^T meantime so the threshold fallback stops repaying the transpose cost.",
        ),
        CandidateConfig(
            strategy="aten",
            main_backend="addmm_inplace",
            ...
            allow_tf32=False,
            cache_mode="hybrid_weff_threshold2_btcache",
            variant_name="aten_addmm_inplace_btcontigout_mainfirst_hybridweff_threshold2_btcache",
            notes="Combine delayed W_eff materialization with B^T caching and an mm_out cold fallback, targeting the weakest hidden case without touching the exact-repeat hot path.",
        ),
    ]
