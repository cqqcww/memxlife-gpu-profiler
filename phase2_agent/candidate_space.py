...
def heuristic_candidates() -> list[CandidateConfig]:
    return [
        ...
        CandidateConfig(
            ...
            allow_tf32=False,
            cache_mode="hybrid_weff_threshold2",
            variant_name="aten_addmm_inplace_btcontigout_mainfirst_hybridweff_threshold2",
            notes="Pure threshold experiment with an mm_out cold fallback: keep single-slot exact-repeat reuse, but delay W_eff materialization and preallocate BX for the cold path.",
        ),
        CandidateConfig(
            strategy="aten",
            main_backend="addmm_inplace",
